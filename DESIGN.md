...
#### 响应式缩放体系
管理后台采用 rem + 视口单位实现整体等比缩放，不同页面机制略有差异：

| 元素 | 策略 | 说明 |
|------|------|------|
| 根字号 | `clamp(13px, 0.35vw + 9px, 15px)` | 以 1440px=14px 为锚点；1280px≈13.5px / 1920px≈15.7px，落差 2px，温和可感知 |
| 全站尺寸单位 | rem | 字号 / 间距 / 卡片 / 按钮统一 rem，随根字号等比缩放 |
| 仪表盘布局 | `grid 1fr` + `@media` 断点 | 卡片按比例铺满；1024px / 640px 断点重排（3列→2列→1列） |
| 列表页布局 | `width: 100%` 铺满 | PageTable 容器跟随内容区，表格 `width:100%`，窄屏横向滚动兜底 |
| 边框 / 阴影 / 字距 | px | 1px 边框、阴影偏移、letter-spacing 等保留 px，避免随缩放失真 |
...
- 统一页面头部：标题 + 描述 + 操作区
- 可选过滤器栏
- 容器 `width: 100%` 铺满内容区，宽屏下表格随宽度拉伸（不设 max-width 限制）
...
| 版本 | 日期 | 说明 |
|------|------|------|
| v1.0.2 | 2026-09-15 | 前端响应式缩放调优：根字号锚定 1440px=14px 窄幅缩放（clamp 13~15px）；PageTable 容器改 width:100% 铺满，消除宽屏右侧留白 |
